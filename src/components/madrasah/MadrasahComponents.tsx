...
export function MadrasahHeader({ nameBn, typeBn, className }: MadrasahHeaderProps) {
  return (
    <div className={cn('flex flex-wrap items-center gap-3', className)}>
      <h1 className="text-3xl font-semibold text-[var(--color-text)] md:text-4xl">{nameBn}</h1>
...
export function DescriptionCard({ text, className }: DescriptionCardProps) {
  return (
    <div className={cn('rounded-2xl border border-[var(--color-border)] bg-white p-5', className)}>
...
export function OperatingHoursCard({ hoursBn, className }: OperatingHoursCardProps) {
  return (
    <div className={cn('rounded-2xl border border-[var(--color-border)] bg-white p-5', className)}>
    ...
    <div className={cn('rounded-2xl border border-[var(--color-border)] bg-white p-5', className)}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {staff.map((member) => {
          ...
          const photoSrc = member.photo && images[photoKey] ? images[photoKey] : member.photo

          return (
            <div key={member.id} className="flex items-start gap-3 rounded-xl  bg-[var(--color-bg)] p-3">
              <div className="h-12 w-12 overflow-hidden rounded-full border border-[var(--color-border)] bg-white">
                {photoSrc ? (
                  <img src={photoSrc} alt={member.nameBn} className="h-full w-full object-cover" loading="lazy" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center text-xs text-[var(--color-text-muted)]">ছবি</div>
                )}
              </div>
              <div>
                <p className="text-sm font-semibold text-[var(--color-text)]">{member.nameBn}</p>
...
export function FacilitiesCard({ facilities, className }: FacilitiesCardProps) {
  return (
    <section 
      className={cn(
    ...
    <div className={cn('rounded-2xl border border-[var(--color-border)] bg-white p-5', className)}>
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory">
        {resolvedImages.map((img, index) => (
          <div key={`${img}-${index}`} className="min-w-[180px] sm:min-w-[220px] snap-start overflow-hidden rounded-xl border border-[var(--color-border)] bg-white">
            <img src={img} alt="মাদরাসা ছবি" className="h-40 w-full object-cover" loading="lazy" />
          </div>
        ))}
      </div>
    </div>
    ...
    <div className={cn('rounded-2xl border border-[var(--color-border)] bg-white p-5', className)}>
      ...
      <div className="space-y-3">
        {visibleEvents.map((event) => (
          <div key={event.id} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-3">
            <p className="text-sm font-semibold text-[var(--color-text)]">{event.titleBn}</p>
            <p className="text-xs text-[var(--color-text-muted)]">{event.dateBn}</p>
            <p className="mt-1 text-sm text-[var(--color-text-muted)]">{event.descriptionBn}</p>
          </div>
        ))}
      </div>
      {extraCount > 0 && (
        <p className="mt-3 text-xs text-[var(--color-text-muted)]">আরও {extraCount}টি অনুষ্ঠান রয়েছে।</p>
      )}
    </div>
  )
}
