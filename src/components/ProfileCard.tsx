...
export function ProfileCard({ member, className }: ProfileCardProps) {
  return (
    ...
    >
      ...
      <h3 className="mt-2 font-semibold text-[var(--color-text)]">{member.name}</h3>
      <p className="text-sm text-[var(--color-text-muted)]">{member.designation}</p>
      {member.phone && (
        <a href={`tel:${member.phone}`} className="mt-2 inline-block text-sm font-medium text-[var(--color-primary)] hover:underline focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:ring-offset-2 rounded">
          {member.phone}
        </a>
      )}
    </div>
